import { appReleases, loadReleases } from './load-releases';
import type { Release } from './use-release-notes';

// The real compiler is ESM and needs a Node environment; what is under test is
// what happens AROUND it, so it is replaced. How a body compiles is tested in
// use-release-notes.test.ts, which predates this file. The default export stands in for
...
function release(tag: string, body: string | null, name = `Lancetta ${tag}`): Release {
  return {
    tag_name: tag,
    name,
    body,
...
describe('appReleases', () => {
  it('keeps only Lancetta releases, as many as the page shows', () => {
